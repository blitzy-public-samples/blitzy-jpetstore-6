...
public class OrderSagaOrchestrator {
    ...
    private Order executeStepCreateOrder(Order order) {
        ...
        // CRITICAL FIX: LineItem uses @IdClass composite PK with orderId + lineNum. When
        // Order has CascadeType.ALL, Hibernate cascades the persist to LineItem entities
        // using their field values — but orderId is a primitive int (default 0) that hasn't
        // been assigned yet (the PostgreSQL sequence generates it on Order INSERT). This
        // causes all cascaded LineItem INSERTs to use order_id=0, violating PK uniqueness.
        //
        // Solution: Detach line items from the Order before saving, save the Order alone to
        // obtain the sequence-generated orderId, then create FRESH LineItem instances with
        // the correct orderId. Using new instances avoids Hibernate's NonUniqueObjectException
        // that occurs when the original transient objects are re-associated with the session
        // after their identity (composite key) changes.
        order.setStatus("PENDING");
        List<LineItem> originalLineItems = new ArrayList<>(order.getLineItems());
        order.getLineItems().clear();

        // Save the order first (without lineItems) to obtain the sequence-generated orderId
        Order savedOrder = orderRepository.saveAndFlush(order);
        int generatedOrderId = savedOrder.getOrderId();
        log.debug("Order saved with generated orderId: {}", generatedOrderId);

        // Create brand-new LineItem instances with the correct orderId to avoid Hibernate
        // session identity conflicts. The original transient objects had orderId=0 and may
        // have been tracked by the persistence context during the Order save. Creating fresh
        // copies ensures clean entity identity with the correct composite PK values.
        List<LineItem> persistedLineItems = new ArrayList<>(originalLineItems.size());
        for (LineItem original : originalLineItems) {
            LineItem fresh = new LineItem();
            fresh.setOrderId(generatedOrderId);
            fresh.setLineNum(original.getLineNum());
            fresh.setItemId(original.getItemId());
            fresh.setQuantity(original.getQuantity());
            fresh.setUnitPrice(original.getUnitPrice());
            persistedLineItems.add(fresh);
        }
        if (!persistedLineItems.isEmpty()) {
            // saveAll returns managed entity copies — capture them to avoid
            // mixing unmanaged transient objects with managed entities in the
            // Order's collection (which would cause NonUniqueObjectException)
            persistedLineItems = new ArrayList<>(lineItemRepository.saveAll(persistedLineItems));
            lineItemRepository.flush();
        }
        // CRITICAL: Use addAll() instead of setLineItems() to preserve the Hibernate-managed
        // collection reference. orphanRemoval=true requires the same collection instance to
        // be maintained — replacing it via setLineItems() triggers HibernateException:
        // "A collection with orphan deletion was no longer referenced by the owning entity"
        savedOrder.getLineItems().addAll(persistedLineItems);
