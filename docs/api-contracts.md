...
> **Note**: The `account.password` field is never included in responses. This matches the monolith behavior where `account.setPassword(null)` is called after successful signon (see `AccountActionBean.signon()` line 169).
...
> **Auto-login**: The response includes a JWT token so the user is automatically authenticated after registration, matching the monolith behavior where `authenticated = true` is set after `insertAccount()` (see `AccountActionBean.newAccount()` line 119).
...
> **Order ID generation**: The `orderId` is generated by a PostgreSQL sequence (`order_id_seq`), replacing the monolith's non-thread-safe `sequence` table. Starting value is set to a value greater than the maximum migrated order ID.
